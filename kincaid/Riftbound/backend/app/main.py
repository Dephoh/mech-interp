...
async def websocket_endpoint(websocket: WebSocket, room_id: str):
    ...
    try:
        ...
        logger.info("[WS] JOIN_ROOM room=%s player_name=%s deck_keys=%s",
                     room_id, player_name, list(deck.keys()) if isinstance(deck, dict) else "BAD")

        session, player_id, slot_idx = await room_manager.join_room(
        ...
        while True:
            ...
            try:
                ...
                logger.exception("[WS] EXCEPTION handling action %s in room=%s player=%s",
